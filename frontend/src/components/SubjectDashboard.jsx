            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {subjects.map((subject) => (
                    <button
                        ...
                        className={`${subject.color} p-10 rounded-[2rem] shadow-sm hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 flex flex-col items-center gap-6 border-4 border-transparent hover:border-white/50 group`}
                    >
                        <span className="text-7xl filter drop-shadow-md group-hover:scale-110 transition-transform duration-300">{subject.icon}</span>
                        <span className="text-3xl font-bold tracking-tight">{subject.name}</span>
                    </button>
                ))}
            </div>
        </div>
