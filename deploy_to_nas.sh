...
ssh $NAS_USER@$NAS_IP "sudo docker pull ghcr.io/eugeneadu/yesmarttutor-backend:latest"

echo ""
echo "📥 2/3: Pulling the latest Frontend Image on NAS..."
ssh $NAS_USER@$NAS_IP "sudo docker pull ghcr.io/eugeneadu/yesmarttutor-frontend:latest"

echo ""
echo "🔄 3/3: Waking up Watchtower to swap out the containers..."
ssh $NAS_USER@$NAS_IP "sudo docker restart yesmarttutor-watchtower"
